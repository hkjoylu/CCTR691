...
> The samples were aligned to multiple genomes, as it shows 73.9% was mapped to hg19 and 17.3% was mapped to mm10
...
> In the summary, there were over 17,000 number of estimated cells, which is a lot more than the expected number. Mean reads per cell is also low at around 10,000 where it is normally expected to be around 20,000.
...
```{r setup}
knitr::opts_knit$set(root.dir = "C:/Users/hkjoy/Documents/CCTR691")
```
...
> Features are genes, counts are reads, and samples are cells. There are 19,612 features meaning genes, and 2575 samples meaning cells
...
> Generally the same range of values but removed many cells that had 0 which shifted the shape of the violin plot with higher densities at 5,000 and 50,000 respectively.
...
> Cell is the x-axis, and Gene is the y axis, meaning each cell is a column with the respective gene expression values in each row based on the gene listed on the y axis
...
> There is more noise and there is decreased organization
...
> The shape of the tSNE plot slightly changed but is largely similar
...
> The plot formed clusters of cells into a linear pattern, and it looks very different from plots with more dimensions
...
> The plotted points are more dispersed with a wider range in both the x and y axes, overall shape is similar to the standard with 1:5 dimension.
